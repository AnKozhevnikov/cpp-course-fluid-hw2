...
template <typename P, typename V, typename VFLOW> class Simulator<P, V, VFLOW, 0, 0>
{
  public:
    ...
    template <typename T> struct VectorFieldDynamic
    {
        array<T, 4> **v;
        VectorFieldDynamic(size_t x, size_t y)
        {
            v = new array<T, 4> *[x];
            for (int i = 0; i < x; i++)
            {
    ...
    void run(std::string source)
    {
        ...
        for (size_t i = 0; i < T; ++i)
        {
                ...
                for (size_t y = 0; y < M; ++y)
                {
                    ...
                    for (auto [dx, dy] : deltas)
                    {
                        ...
                        if (field[nx][ny] != '#' && old_p[nx][ny] < old_p[x][y])
                        {
                            ...
                            contr = 0;
                            velocity.add(x, y, dx, dy, force / P(rho[(int)field[x][y]]));
            ...
            VectorFieldDynamic<VFLOW> nvelocity_flow(N, M);
            swap(velocity_flow, nvelocity_flow);
            bool prop = false;
            do
            {
                UT += 2;
